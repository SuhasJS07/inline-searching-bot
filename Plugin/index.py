...
async def index_files(bot, query):
    ...
    if raju == 'reject':
        ...
        await bot.send_message(int(from_user),
                               f'Darling ! Your Submission for indexing {chat} from moderators made declined .',
                               reply_to_message_id=int(lst_msg_id))
        return

    if lock.locked():
        return await query.answer('Ruko zraa! sabr kro, show_alert=True)
    msg = query.message

    await query.answer('raste mei hai...⏳', show_alert=True)
    if int(from_user) not in ADMINS:
        await bot.send_message(int(from_user),
                               f' Congrats darling! Your Submission for indexing {chat} from moderators  accepted see in the index  .',
                               reply_to_message_id=int(lst_msg_id))
    await msg.edit(
        "Starting Indexing",
...
async def send_for_index(bot, message):
    ...
    try:
        ...
        return await message.reply('hey , this is private channel / group. \nFiles for index please make me Admin of the group ❤️.')
    ...
    try:
        ...
        return await message.reply('Hey is that your channel is private please check that🙆🏻‍♀️ !! ')
    if k.empty:
